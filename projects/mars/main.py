...
def main(args):    
    ...
    if os.path.exists(args.scenario):
        ...
        model_library_path = os.path.join(abs_dir, 'model_classes')

    scenario = None
    with open(abs_path, 'r') as f:
        scenario = json.load(f)
    t0 = time.time()
    outputs = run_scenario(scenario, model_library_path)
    if 'error' in outputs:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--scenario', type=str, help='Name or path to scenario to run', default='solar_only')
